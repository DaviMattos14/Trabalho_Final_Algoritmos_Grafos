...
export function loadGraphToEditor(adjGraph) {
    ...
    const keys = Object.keys(adjGraph).sort((a, b) => {
    ...
    // Layout Tree
    const root = keys[0];
    // Como calculateLayout agora é exportado, isso deve funcionar
    ...
    keys.forEach(key => {
        if (treePositions[key]) {
            nodes.push({ id: key, x: treePositions[key].x + offsetX, y: treePositions[key].y + offsetY });
        } else {
            nodes.push({ id: key, x: 100, y: 100 });
        }
    });

    keys.forEach(u => {
        adjGraph[u].forEach(v => {
...
export function exportGraphFromEditor() {
    ...
    edges.forEach(edge => {
        if (adjGraph[edge.from] && adjGraph[edge.to]) {
            if (!adjGraph[edge.from].includes(edge.to)) {
                adjGraph[edge.from].push(edge.to);
            }
        }
    });
    return adjGraph;
...
function drawEditor() {
    ...
    nodes.forEach(node => {
        ...
        if (node === movingNode) ctx.fillStyle = "#e3f2fd"; 
        else if (node === dragStartNode) ctx.fillStyle = "#bbdefb"; 
...
function getDistanceToSegment(p, v, w) { return Math.sqrt(getDistanceToSegmentSquared(p, v, w)); }
function getEdgeAt(x, y) {
    const clickPoint = {x, y}; const tolerance = 8; 
    return edges.find(edge => {
        const n1 = nodes.find(n => n.id === edge.from);
        const n2 = nodes.find(n => n.id === edge.to);
        if (!n1 || !n2) return false;
        const dist = getDistanceToSegment(clickPoint, n1, n2);
        return dist < tolerance;
    });
}
function getNextId() {
    let id = 0;
    while (nodes.some(n => n.id === id.toString())) { id++; }
    return id.toString();
}
...
canvas.addEventListener('mousedown', (e) => {
    ...
    if (e.button === 0) {
        if (clickedNode) { isDraggingLine = true; dragStartNode = clickedNode; }
            ...
            const nearby = nodes.find(n => Math.sqrt((n.x - x)**2 + (n.y - y)**2) < 40);
            if (!nearby) { nodes.push({ id: getNextId(), x: x, y: y }); drawEditor(); }
        }
    }
    ...
    if (e.button === 2) {
        ...
        if (now - lastRightClickTime < 300) {
            ...
            if (clickedEdge) { edges = edges.filter(edge => edge !== clickedEdge); drawEditor(); return; }
        }
        lastRightClickTime = now;
        if (clickedNode) { isMovingNode = true; movingNode = clickedNode; moveOffset.x = x - clickedNode.x; moveOffset.y = y - clickedNode.y; canvas.style.cursor = "grabbing"; drawEditor(); }
    }
});
canvas.addEventListener('mousemove', (e) => {
    ...
    if (isMovingNode && movingNode) {
        ...
        movingNode.x = Math.max(20, Math.min(canvas.width - 20, movingNode.x));
        movingNode.y = Math.max(20, Math.min(canvas.height - 20, movingNode.y));
        drawEditor(); return;
    }
    const hoverNode = getNodeAt(x, y);
    if (hoverNode) { canvas.style.cursor = isMovingNode ? "grabbing" : "grab"; }
    else { const hoverEdge = getEdgeAt(x, y); canvas.style.cursor = hoverEdge ? "pointer" : "crosshair"; }
});
canvas.addEventListener('mouseup', (e) => {
    if (e.button === 0 && isDraggingLine) {
        ...
        if (targetNode && targetNode !== dragStartNode) {
            const exists = edges.some(edge => edge.from === dragStartNode.id && edge.to === targetNode.id);
            if (!exists) { edges.push({ from: dragStartNode.id, to: targetNode.id }); }
        }
